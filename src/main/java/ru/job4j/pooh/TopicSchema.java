        ...
        while (!Thread.currentThread().isInterrupted()) {
            for (String key : data.keySet()) {
                if (receivers.containsKey(key)) {
                    BlockingQueue<String> dataQueue = data.get(key);
                    CopyOnWriteArrayList<Receiver> receiveArray = receivers.get(key);
                    while (!dataQueue.isEmpty()) {
                        String text = dataQueue.poll();
                        for (Receiver e : receiveArray) {
                            e.receive(text);
                        }
                    }
                }
